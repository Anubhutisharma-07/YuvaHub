  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto space-y-8 font-sans pb-16 px-2 sm:px-4">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3">
            ...
            <h1 className="text-2xl md:text-3xl font-serif font-bold text-[#231f20] dark:text-white tracking-tight">
              Career Match <span className="text-[#b56b37] italic">Studio</span>
            </h1>
          ...
          <div className="flex items-center gap-4 bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 p-4 rounded-2xl w-full lg:w-auto shadow-xs">
            ...
            <div>
              <div className="text-[10px] uppercase font-bold text-[#8c7569] tracking-wider">Role Match Score</div>
              <div className="text-xs font-extrabold text-[#231f20] dark:text-white">{currentRole.title}</div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar border-b border-[#e8ded1] dark:border-slate-800 pb-3">
        {[
          ...
          return (
            ...
            >
              <IconComponent className={`w-3.5 h-3.5 ${isActive ? 'text-[#f3e4bd]' : 'text-[#b56b37]'}`} />
              <span>{dom.label}</span>
            </button>
          );
        })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 shadow-2xs">
            <div className="border-b border-[#e8ded1] dark:border-slate-800 pb-4">
              <h2 className="text-xl font-serif font-bold text-[#231f20] dark:text-white">Your Technical Stack</h2>
              <p className="text-xs text-[#603620] dark:text-slate-400 font-medium mt-1">Add or remove skills to dynamically recalculate your role fit score.</p>
            </div>
            ...
            <form onSubmit={handleAddSkill} className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#fcf9f2] dark:bg-slate-800 border border-[#e8ded1] dark:border-slate-700 rounded-xl p-3 text-xs text-[#231f20] dark:text-white outline-none"
              />
              <button type="submit" className="px-5 py-3 bg-[#b56b37] hover:bg-[#96552a] text-white font-bold text-xs rounded-xl shadow-xs cursor-pointer flex items-center gap-1.5">
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              {userSkills.map(skill => (
                <span key={skill} className="px-3 py-1.5 bg-[#f6efe2] dark:bg-slate-800 text-[#231f20] dark:text-slate-200 border border-[#e8ded1] dark:border-slate-700 rounded-xl text-xs font-bold flex items-center gap-2 shadow-2xs">
                  <span>{skill}</span>
                  <button onClick={() => handleRemoveSkill(skill)} className="text-[#8c7569] hover:text-red-600 transition-colors">
          ...
          <div className="bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 shadow-2xs">
            <div className="border-b border-[#e8ded1] dark:border-slate-800 pb-4 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-serif font-bold text-[#231f20] dark:text-white">Role Match Audit</h2>
                <p className="text-xs text-[#603620] dark:text-slate-400 font-medium mt-1">{currentRole.title}</p>
              </div>
              <span className="text-xs font-bold text-[#b56b37] bg-[#f3e4bd] px-3 py-1 rounded-full border border-[#e8ded1]">
      ...
      {activeTab === 'roadmap' && (
        <div className="bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 shadow-2xs">
          <div className="border-b border-[#e8ded1] dark:border-slate-800 pb-4">
            <h2 className="text-xl font-serif font-bold text-[#231f20] dark:text-white">Recommended Skill Upgrade Path</h2>
            <p className="text-xs text-[#603620] dark:text-slate-400 font-medium mt-1">Focus on these missing skills to reach 100% role readiness for {currentRole.title}.</p>
          </div>

          <div className="space-y-4">
            {dynamicMatchResult.missing.map((gap, idx) => (
              <div key={gap} className="p-5 rounded-2xl bg-[#fcf9f2] dark:bg-slate-800/50 border border-[#e8ded1] dark:border-slate-700 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-serif font-bold text-base text-[#231f20] dark:text-white">{gap}</h3>
                    <p className="text-xs text-[#603620] font-medium">Recommended: Complete 2 hands-on projects using {gap}.</p>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-[#b56b37] hover:bg-[#96552a] text-white text-xs font-bold rounded-xl cursor-pointer shadow-2xs"
                >
                  Mark Mastered
                </button>
              </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredTeammates.map(tm => (
              <div key={tm.id} className="bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 rounded-2xl p-5 shadow-2xs space-y-4 hover:border-[#b56b37] transition-all">
                <div className="flex justify-between items-start">
                  <div>
                    <h3 className="font-serif font-bold text-base text-[#231f20] dark:text-white">{tm.name}</h3>
                    <p className="text-xs text-[#b56b37] font-bold mt-0.5">{tm.role}</p>
                    <p className="text-[11px] text-[#8c7569] font-medium">{tm.college}</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tm.skills.map(s => (
                    <span key={s} className="px-2 py-0.5 bg-[#f6efe2] text-[#603620] text-[10px] font-bold rounded-md border border-[#e8ded1]">
                      #{s}
                    </span>
                  ))}
                </div>

                <div className="pt-3 border-t border-[#e8ded1] dark:border-slate-800 text-xs font-semibold text-[#603620]">
      ...
      {activeTab === 'export' && (
        <div className="bg-white dark:bg-slate-900 border border-[#e8ded1] dark:border-slate-800 rounded-3xl p-8 text-center space-y-4 shadow-2xs max-w-xl mx-auto">
          ...
          <h2 className="text-2xl font-serif font-bold text-[#231f20] dark:text-white">Export Career Match Telemetry</h2>
