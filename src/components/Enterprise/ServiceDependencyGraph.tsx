...
export const ServiceDependencyGraph: React.FC<Props> = ({ services }) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-6">
        <div className="flex justify-between items-center border-b border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Interactive node map detailing downstream dependencies.</p>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <code className="text-xs text-slate-500">{svc.id}</code>
                </div>
              </div>

              {/* Dependencies Badges */}
              <div className="flex items-center gap-2 flex-wrap text-xs">
                <span className="text-slate-500 font-semibold">Depends on:</span>
                {svc.dependencies.length > 0 ? (
                  svc.dependencies.map((depId) => (
                    <span
                      key={depId}
                      className="px-2 py-0.5 rounded-lg bg-slate-800 text-indigo-300 font-mono text-[11px] border border-slate-700 flex items-center gap-1"
                    >
                  ...
                  <span className="text-slate-600 italic">None (Root Node)</span>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="p-6 bg-slate-900 border border-slate-800 rounded-3xl space-y-6 flex flex-col justify-between">
        {selectedService ? (
          <div className="space-y-6">
            <div className="border-b border-slate-800 pb-4 flex justify-between items-start">
              ...
              <span className={`px-2.5 py-1 text-xs font-bold rounded-full uppercase ${
                selectedService.status === 'operational' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' :
                selectedService.status === 'degraded' ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' :
            ...
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-3 text-xs">
                <div className="flex justify-between">
                  <span className="text-slate-400 font-medium">24-Hour Uptime:</span>
              ...
              <div>
                <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Downstream Edge Dependents</div>
                <div className="space-y-1.5">
                  {services.filter(s => s.dependencies.includes(selectedService.id)).length > 0 ? (
                    services.filter(s => s.dependencies.includes(selectedService.id)).map(dep => (
                      <div key={dep.id} className="p-2.5 rounded-xl bg-slate-950 border border-slate-800/60 text-xs font-medium text-slate-300 flex justify-between items-center">
                        <span>{dep.name}</span>
                        <code className="text-[10px] text-indigo-400">{dep.id}</code>
                      </div>
                    ))
                  ) : (
                    <p className="text-xs text-slate-500 italic">No downstream services depend on this node.</p>
                  )}
                </div>
              </div>
            </div>
