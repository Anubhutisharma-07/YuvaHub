...
import { cacheMiddleware } from "./src/api/middlewares/cacheMiddleware.js";
import { redisClient, createFailOpenStore } from "./src/api/redis.js";
import { resumeRateLimiter } from "./src/api/middlewares/rateLimiter.js";
import {
  createApplication,
  confirmApplication,
  updateApplicationStatus as updateApplicationTracker,
  ...
  try {
    ...
    const application = await createApplication({
      ...
      platform:
        ...
        opportunity.source_name ||
        "unknown",
      status: (status as any) || "pending_confirmation",
      userConfirmed: false,
    });
...
app.patch("/api/v1/applications/:id", async (req, res) => {
  try {
    ...
    await updateApplicationStatus(
      id,
      (status as any) || "pending_confirmation",
      notes
    );
...
app.get("/api/v1/opportunities/semantic-search", async (req: Request, res: Response) => {
  try {
    if (!dbQuery) {
  ...
  try {
    ...
    if (existingUser) {
      const updateData: any = {
        ...
        onboarded: req.body.onboarded !== undefined
          ? req.body.onboarded
          : (existingUser.onboarded ?? Boolean(existingUser.college || existingUser.field || existingUser.year)),
